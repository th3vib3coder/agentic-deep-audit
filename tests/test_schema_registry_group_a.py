...
def test_risk_schema_rejects_unconfirmed_heuristic_source_kind() -> None:
    valid = {"schema_version": "1.0", "findings": [{"finding_id": "risk-1", "category": "dependency_cve", "severity": "high", "confidence": "high", "evidence_ids": ["ev-1"], "source_kind": "external_tool", "recommendation": "upgrade", "requires_human_decision": False}]}
...
@pytest.mark.parametrize(
    ("schema_name", "sample"),
    [
        ("suspicious_behaviors.schema.json", {"schema_version": "1.0", "behaviors": [{"behavior_id": "susp-1", "kind": "eval", "evidence_ids": ["ev-1"], "severity": "high", "confidence": "medium", "source": "heuristic"}]}),
        ("agentic_security_finding.schema.json", {"schema_version": "1.0", "findings": [{"finding_id": "agentic-1", "code": "W015", "target": "AGENTS.md", "severity": "medium", "confidence": "high", "evidence_ids": ["ev-1"], "recommendation": "sanitize"}]}),
        ("license_cards.schema.json", {"schema_version": "1.0", "cards": [{"card_id": "lic-1", "kind": "file", "declared_license": "MIT", "confidence": "medium", "detection_method": "spdx_header", "requires_human_decision": False, "evidence_ids": ["ev-1"]}]}),
        ("reuse_cards.schema.json", {"schema_version": "1.0", "target_context": {"allowed_languages": ["python"]}, "scoring_input_sources": {"special_implementations": "SPECIAL_IMPLEMENTATIONS.json"}, "cards": [{"reuse_id": "reuse-1", "candidate_id": "candidate-1", "name": "Indexer", "problem_solved": "search", "files": ["a.py"], "license_status": "needs_review", "target_context": {"allowed_languages": ["python"]}, "recommendation": "study", "decision": "study", "requires_human_decision": True, "legal_caveat": "review", "security_caveat": "review", "performance_caveat": "static", "score_inputs": {}, "evidence_ids": ["ev-1"]}]}),
        ("scientific_provenance.schema.json", {"schema_version": "1.0", "records": [{"record_id": "sci-1", "category": "tool_version", "observed_value": {"tool": "nextflow"}, "evidence_ids": ["ev-1"], "confidence": "high", "detection_method": "manifest_or_lockfile", "requires_human_decision": False}]}),
        ...
        ("binary_artifacts.schema.json", {"schema_version": "1.0", "artifacts": [{"artifact_id": "bin-1", "path": "a.dll", "kind": "native_binary", "size_bytes": 1, "triage_triggered": True, "requires_operator_consent": True, "evidence_ids": ["ev-1"]}], "summary": {"blob_count": 1, "total_binary_bytes": 1, "native_binary_count": 1, "mime_mismatch_count": 0}}),
...
def test_scientific_provenance_records_require_evidence() -> None:
    valid = {"schema_version": "1.0", "records": [{"record_id": "sci-1", "category": "tool_version", "observed_value": {"tool": "nextflow"}, "evidence_ids": ["ev-1"], "confidence": "high", "detection_method": "manifest_or_lockfile", "requires_human_decision": False}]}
